...
<head>
    ...
    <title>{{ $title ?? 'Dashboard' }} | {{ config('app.name', 'Sistem Kas Sederhana Multi-User') }}</title>
    ...
    <div class="min-h-screen xl:flex bg-gray-50 dark:bg-gray-900">
        ...
        <div class="flex-1 transition-all duration-300 ease-in-out"
            ...
            <div class="p-4 mx-auto max-w-(--breakpoint-2xl) md:p-6 space-y-6">
                ...
                @if (session('warning'))
                    <x-alerts.alert type="warning" :message="session('warning')" />
                @endif

                @if (session('info'))
                    <x-alerts.alert type="info" :message="session('info')" />
                @endif
