...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">
                Edit Kategori Kas: {{ $category->name }}
            </h1>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 sm:p-8 shadow-xs">
        <form method="POST" action="{{ route('modules.cash.categories.update', $category) }}" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <x-forms.select
                    ...
                    :selected="old('type', $category->type->value)"
                    required
                >
                    <x-slot:iconSlot>
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
