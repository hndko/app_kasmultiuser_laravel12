...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 sm:p-8 shadow-xs">
        <form method="POST" action="{{ route('modules.users.store') }}" enctype="multipart/form-data" class="space-y-5">
            @csrf
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                >
                    ...
                    @foreach ($roles as $role)
                        <option value="{{ $role->value }}" {{ old('role', 'user') === $role->value ? 'selected' : '' }}>
                            {{ $role->label() }}
                        </option>
                    @endforeach
                </x-forms.select>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <!-- Password with Icon Group & Placeholder -->
                <x-forms.input
                    type="password"
                    name="password"
                    label="Kata Sandi"
